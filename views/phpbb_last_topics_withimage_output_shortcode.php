...
// to center image thinkbox iframe
//onclick="function w3centerImg(){var ifr=document.getElementById(\'TB_iframeContent\').contentDocument.body.setAttribute(\'style\',\'display:flex;justify-content:center\');}setTimeout(function(){w3centerImg();},500);return false;"
echo '<a onclick="function w3centerImg(){var ifr=document.getElementById(\'TB_iframeContent\').contentDocument.body.setAttribute(\'style\',\'display:flex;justify-content:center\');}setTimeout(function(){w3centerImg();},500);return false;" style="border:0;margin:0;text-decoration:none;color:transparent" href="'.$w3all_url_to_cms.'/download/file.php?id='.$w3topic->attach_id.'&TB_iframe=true" class="thickbox"><img style="width:100%" alt="'.$w3topic->topic_title.'" src="'.$w3all_url_to_cms.'/download/file.php?id='.$w3topic->attach_id.'" /></a>';
if($img_info_right_or_bottom == 'bottom'){
echo '<div class="'.$w3LSIMG_DIV0.'">'
.$titleLinkToPost;
...
echo'</td><!-- close td left - if image -->';
} else {
...
echo'</td><!-- close td left - if not an image -->';
}

if($img_info_right_or_bottom == 'right'){
...
} # END if($img_info__right_bottom == 'right'){
