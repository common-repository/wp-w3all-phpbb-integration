...
function wp_w3all_phpbb_iframe_short( $atts ){
 ...
 if(is_array($atts)){
  ...
  if( $w3all_custom_output_files == 1 ) {
    if (file_exists(ABSPATH.'wp-content/plugins/wp-w3all-custom/wp_w3all_phpbb_iframe_short.php')){
     ob_start();
      include(ABSPATH.'wp-content/plugins/wp-w3all-custom/wp_w3all_phpbb_iframe_short.php');
     return ob_get_clean();
    } else {
      ob_start();
       include(WPW3ALL_PLUGIN_DIR . 'views/wp_w3all_phpbb_iframe_short.php');
      return ob_get_clean();
     }
  } else {
      ob_start();
...
function wp_w3all_short_add_css_ajax() {
  global $w3all_url_to_cms, $wp_w3all_forum_folder_wp,$w3all_iframe_phpbb_link_yn, $w3allhomeurl, $w3all_iframeResizerVersion;

          $w3all_url_to_phpbb_ib = $w3all_url_to_cms . "/ucp.php?i=pm&folder=inbox";

$s = "<script type=\"text/javascript\" src=\"".plugins_url()."/wp-w3all-phpbb-integration/addons/iframeresizer/iframe-resizer.parent.js\"></script>
