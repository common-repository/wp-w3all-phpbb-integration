...
if ( ! empty( $user_query->results ) ) {
 ...
 if (empty($urank_id_a)){
   foreach($phpbb_group as $kv){
    ...
    $urank_id_a = $kv;
    goto w3this1; // break to the first found ('it seem' the default phpBB behavior)
    }
   }}
 }
w3this1:
...
if( $wpu->ID > 1 && is_email($wpu->user_email) ){
       ...
       $wpunn = esc_sql(mb_strtolower($wpu->user_login));
       $wpup = $wpu->user_pass;
       $wpue = $wpu->user_email;
       $uavatar = $avatype = '';
   ...
   $user_exist = $phpbb_conn->get_results("SELECT * FROM ".$phpbb_config_file["table_prefix"]."users WHERE lower(username) = '$wpul' OR lower(user_email) = '$wpue' AND lower(username) != '$wpul'");
    ...
    if(! empty($user_exist) )
    {
     foreach($user_exist as $ue){
      if ( strtolower($ue->user_email) == strtolower($wpue) && mb_strtolower($ue->username) != mb_strtolower($wpul) ){
